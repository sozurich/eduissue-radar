...
# 5. Simple local summarizer
def summarize_local(text, max_sentences=5):
    # split sentences
    sentences = re.split(r'(?<=[.!?])\s+', text)
    # word frequency
    words = re.findall(r'[\w가-힣]+', text.lower())
    stopwords = set(["이거","그거","저희","합니다","입니다","그리고","하지만"])
    freq = Counter(w for w in words if len(w)>1 and w not in stopwords)
    # score sentences
    scores = []
    for i,s in enumerate(sentences):
        s_words = re.findall(r'[\w가-힣]+', s.lower())
    ...
    return summary if summary else "로컬 요약을 위한 충분한 텍스트가 없습니다."
...
if uploaded:
    ...
    with tab3:
        ...
        st.markdown("---")
        st.subheader("🔹 로컬 요약 (추출식)")
        summary_local = summarize_local(text, max_sentences=5)
